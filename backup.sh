...
# ─────────────────────────────────────────────────────────
# 1. STOP DE VM (met graceful service shutdown)
# ─────────────────────────────────────────────────────────
...
if pgrep -f "microvm@hermes-agent" > /dev/null 2>&1; then
    ...
    if [[ -S "$SANDBOX/control.sock" ]]; then
        ...
        curl -sf --unix-socket "$SANDBOX/control.sock" \
            ...
            -d '{"cmd": ["systemctl", "stop", "hermes-gateway", "hermes-agent", "hermes-dashboard"]}' \
            && log "  Stop signaal verstuurd"

        # Wacht maximaal 30 seconden
        log "  Wachten op service shutdown (max 30s)..."
        SERVICES_STOPPED=false

        for i in $(seq 1 30); do
            SERVICES_RUNNING=0

            # Check hermes-gateway (poort 8644)
            curl -sf --unix-socket "$SANDBOX/control.sock" \
                -X PUT http://localhost/vm.exec -H "Content-Type: application/json" \
                -d '{"cmd": ["timeout", "1", "bash", "-c", "echo > /dev/tcp/127.0.0.1/8644"]}' > /dev/null 2>&1 && SERVICES_RUNNING=$((SERVICES_RUNNING + 1))

            # Check hermes-dashboard (poort 3333)
            curl -sf --unix-socket "$SANDBOX/control.sock" \
                -X PUT http://localhost/vm.exec -H "Content-Type: application/json" \
            ...
            if [[ $i -eq 15 ]]; then
                ...
                curl -sf --unix-socket "$SANDBOX/control.sock" \
                    -X PUT http://localhost/vm.exec -H "Content-Type: application/json" \
                    -d '{"cmd": ["systemctl", "kill", "-s", "SIGKILL", "hermes-gateway", "hermes-agent", "hermes-dashboard"]}' > /dev/null 2>&1
            fi
        done

        if [[ "$SERVICES_STOPPED" == false ]]; then
            log "  Service shutdown timeout — ga door met VM shutdown"
        fi
    fi
